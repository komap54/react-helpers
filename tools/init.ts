...
const modifyFiles = [
  'package.json',
  'rollup.config.ts',
  'test/awesome-library.test.ts',
  'tools/gh-pages-publish.ts',
];
const renameFiles = [
  ['src/awesome-library.ts', 'src/awesome-library.ts'],
  ['test/awesome-library.test.ts', 'test/awesome-library.test.ts'],
];
 ...
 * has been cloned into a custom directory (i.e. it's not 'awesome-library')
 */
...
/**
 * Checks if the suggested library name is the default, which is 'awesome-library'
 */
function libraryNameSuggestedIsDefault() {
  if (libraryNameSuggested() === 'awesome-library') {
    return true;
  }
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/awesome-library/g, /--username--/g, /--usermail--/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function (files) {
    ...
    const newFilename = files[1].replace(/awesome-library/g, libraryName);
    mv(
